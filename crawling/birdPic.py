...
def save_data(detail):
    ...
    try:
        ...
        sql = 'insert into birdpic_map(name,date,location,province,city,district,imgsrc) ' \
              'values(%s,%s,%s,%s,%s,%s,%s)'
        cursor1.execute(sql, (detail['name'], detail['date'], detail['location'],detail['province'],detail['city'],detail['district'],detail['imgsrc']))
        # 提交事务
        connection.commit()
        print("%d记录保存成功\n" % (number))
        number+=1
...
picUrl = "http://www.birder.cn/photo.html?&page=1"
def Crawl():
    pageIndex = 1
    End = False
    while not End:
        picUrl = "http://www.birder.cn/photo.html?&page=%d"%(pageIndex)
        html = getHtml(picUrl)
        if html:
            soup = BeautifulSoup(html,'html.parser')
...
Crawl()
# getRecord('http://www.birder.cn/photo/Corvus-corax-2016-10-09-82.html')